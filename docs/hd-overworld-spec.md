...
状态：产品方向已锁定；占位素材已进本分支。**P0 表现壳已在本分支落地（partial）**：`classic` / `hd-map` 切换、`#hd-overworld` 1080p 层、地形+城标、点击入城尽力而为。缺口见 §9 与 FEATURES.md。
...
探测代码已进 `js/hd-overworld-probe.js`（只读）。切到 HD 地图后第一次抽样会打印字段候选与 38 城坐标表。把控制台结果补进本节即可，不要改 WASM。保留 `LICENSE` / `LICENSE.ENGINE` 与上游注释。
