...
- 不把战斗地图、标题/存档先做成 HD（那些是后续轨道）。城池四项菜单已另开 [hd-city-menu-spec.md](hd-city-menu-spec.md)。
...
- 关菜单：只在 HD 城池壳开着时对引擎发 EXIT。地图上再 `leaveMenu` 会退出 `GetCitySet` 掉进「策略结束」，地图点不了城。若已在功能菜单，入城前只 EXIT 一次回到 `PlayerTactic`。
