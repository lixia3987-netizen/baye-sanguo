...
**素材现状（分支内）：** `assets/hd-overworld/` 地形已换成 **史实向地理包**（见同目录 `GEOGRAPHY.md`）：西左北上，黄河偏北、长江偏南、秦岭东西向等，与引擎城格相对方位对齐，**不是**经纬度 1:1。城标仍只读 `g_CityPositions`，不随底图挪动。其它层仍是 AI 占位。**不是**步步高原作美术。  
...
- 绘制顺序：地形（史实向 plains/mountains/rivers/forest，1920×1080）→ 路（5px 土色二次曲线）→ 城标/势力 → 城名 → 悬停/选中/入城闪（P3）。城点不随地形重绘移动
