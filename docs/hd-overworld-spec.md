...
状态：产品方向已锁定；占位素材已进本分支。**P0 容器 + P1 城态/点选已在本分支落地（P1 partial）**：四态城标与势力色、城名、点城开经典菜单（城间方向键逼近 + 可写光标字段）。年月 HUD 探测到才显示，否则占位。缺口见 §8–§9 与 FEATURES.md。
...
- P1：归属色随 `Belong` 变；点己方可入城出菜单。 **壳侧已做；入城对齐仍可能对不齐，见下。**

P1 已实现：

- 四态：`empty`（Belong 0 / 0xff）/ `owned`（Belong === `g_PlayerKing+1`）/ `neutral`（其它势力，色环按 `factions.json` 24 槽哈希）/ `selected`（`marker_selected` + 脉动）
...
- 引擎若没有可写当前城字段，邻城路径假设「方向键跳到该方向最近城」。若原作不是这样，仍可能进错城 → 切回经典
- 年月字段以运行时探测为准，见 §8；未找到时不猜 190 年
- 选君主形势图仍可能被当成大地图（P0 启发式未改）
- P2：路网与地形层次可读，仍无假截图。
...
| **本文** | 真 HD 大地图的下一产品轨道 |
| `js/hd-overworld.js` / `css/hd-overworld.css` | P0 表现壳（本分支已建立） |
