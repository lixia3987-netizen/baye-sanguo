...
9. 出征向导步骤钉死：`persons → food → target-tip → map-pick → march-ok`。残留 `g_hdReportGbk`「选择目标」、换城、过月策略结束不能把选将阶段的点城/回车打进将领表。过月残留人物台词在打开出征 / 完成选将时关壳，不回车进 FunctionMenu；leftover `g_hdMapPick` 不当活 GetCitySet，不挡住「完成选将」和 GetFood。面板标明当前步骤。安定 / 天水各走两遍 noisy 出征到横幅 + 策略结束 + GamFight。
