...
验证环境：`python3 -m http.server` 打开 `index.html` → `choose.html` → `pc.html`，使用「三国霸业-词典原版」`libs/dat-mod.lib`。
...
| 功能 | 状态 | 说明 |
|------|------|------|
| 主菜单：新君登基 / 重返沙场 / 制作群组 / 解甲归田 | done | `gamEng.c` `mainMenuButtonRects`；需在浏览器走通 |
| 时代：董卓弄权 / 曹操崛起 / 赤壁之战 / 三国鼎立 | done | `periodMenuButtonRects` |
| 君主选择 | done | 引擎「势力形势图」选君主 |
| 开场动画 | done | `showStartMovie` |
| 大地图（12×9 / 38 城） | done | `citiesCount` / `cityMapWidth` / `cityMapHeight` |
| 城市光标移动、入城 | done | 大地图操作 |
...
开垦、招商、搜寻、治理、出巡、招降、处斩、流放、赏赐、没收、交易、宴请、输送、移动 — **done**（`MENU_INTERIORSTR` + `order.h`）。

### 外交

离间、招揽、策反、反间、劝降 — **done**（`MENU_DIPLOMATISMSTR`）。朝贡在源码中已注释，原作亦无独立菜单项。

### 军备

侦察、征兵、分配、掠夺、出征 — **done**（`MENU_ARMAMENTSTR`）。
...
3. **战斗全流程**：开局与城池菜单已在浏览器验证；完整一场战斗依赖随机遇敌或主动出征，耗时较长，引擎指令与兵种/天气/地形均在，标记为引擎完成。若需补测可从军备「出征」进入。
