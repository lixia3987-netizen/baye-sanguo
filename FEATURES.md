...
| 点击入城 | partial | 走格只认 `setx/sety`。`g_hdMapPick` 过图与出征共用：只在出征向导里点城当目标；他方城不回车（避免「敌方城池」挡层） |
...
| HD 城池四项菜单 M0–M3 | done | 根+一层+状况+人物/城/数量。标题跟本次入城（点中/落地城），不沿用上场「西凉」。出征：点将后 EXIT 才进粮草/GetCitySet。BattleMake 期间除一次「完成选将」外不发 EXIT；漏点完成选将亮红提示；「部队已出发」大横幅 |
...
| HD 战场 B0/B1 | partial | `js/hd-battle.js`：fight hook / `g_FgtParam` 检测；有 `g_GenPos` 则画单位。新 `GamFight` / 策略结束清 `g_hdFightOver`/`active`/`wait`/`resultDismissed`，第二场不沿用上场覆没。战场系统须本场见过选将 + 新鲜 `onMenuIdle` + `wait=0`；系统菜单等 `wait=1` 再 EXIT。结算后关壳，不挡招商 |
